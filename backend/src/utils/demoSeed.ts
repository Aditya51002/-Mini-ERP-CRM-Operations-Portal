...
async function main(): Promise<void> {
  ...
  const customerData = [
    {
      name: "Rohit Malhotra",
      mobile: "9876500001",
      email: "rohit@malhotratraders.in",
      businessName: "Malhotra Traders",
      gstNumber: "07AAAPM1234C1Z5",
    ...
    },
    {
      ...
      address: "MG Road, Jalandhar",
      status: "LEAD" as const,
      followUpDate: new Date(Date.now() + 2 * 86400000)
    },
    {
      name: "Vikram Distributors",
      mobile: "9876500003",
      email: "contact@vikramdist.com",
      businessName: "Vikram Distributors Pvt Ltd",
      gstNumber: "03BBBPD5678E1Z2",
    ...
    data: [
      {
        ...
        note: "Called regarding bulk order for next month. Interested in 500+ units of steel fasteners.",
        createdById: sales.id
      },
      {
        customerId: customers[1].id,
        note: "First contact — walked into showroom asking about retail pricing. Follow up before Friday.",
        createdById: sales.id
      },
      {
        customerId: customers[2].id,
        note: "Long-term distributor, pays on time. Renewed annual agreement.",
        createdById: sales.id
      }
    ]
  });
  ...
  for (const i of confirmedItems) {
    ...
    await prisma.stockMovement.create({
      data: {
        ...
        quantity: i.quantity,
        movementType: "OUT",
        reason: `Challan #${confirmed.challanNumber} confirmed`,
        createdById: sales.id
      }
    });
  }
  ...
  await prisma.stockMovement.create({
    data: {
      productId: cancelledItems[0].product.id,
      quantity: cancelledItems[0].quantity,
      movementType: "OUT",
      reason: `Challan #${cancelled.challanNumber} confirmed`,
      createdById: sales.id
    }
  });
  await prisma.stockMovement.create({
    data: {
      ...
      quantity: cancelledItems[0].quantity,
      movementType: "IN",
      reason: `Challan #${cancelled.challanNumber} cancelled`,
  ...
  await prisma.purchaseOrder.create({
    data: {
      ...
      createdById: warehouse.id,
      items: {
        create: [{ productId: products[0].id, quantity: 200, unitCostSnapshot: "12.50" }]
      }
    }
  });
